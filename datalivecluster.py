...
def getTopRatedRecipe(recipes, allrecipes):
	#DESCRIPTION: 
	...
	cmd = "SELECT id, rating, imgurl, imgurllg, sourcename FROM records WHERE id IN (\'"+"\',\'".join(allrecipes)+"\') ORDER BY rating DESC"
	cursor.execute(cmd)
...
def findNameGroups(searchresults, recordsHash):
